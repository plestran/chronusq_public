...
void SingleSlater<T>::iniSingleSlater(Molecule * molecule, BasisSet * basisset, 
  ...
  try { 
    this->epsA_ = std::unique_ptr<RealMatrix>(new TMatrix(this->nTCS_*this->nBasis_,1)); 
  } catch (...) { 
  ...
  if(!this->isClosedShell && this->Ref_ != TCS) {
    ...
    try { this->epsB_       = std::unique_ptr<RealMatrix>(new TMatrix(this->nBasis_,1)); } // Beta Eigenorbital Energies
