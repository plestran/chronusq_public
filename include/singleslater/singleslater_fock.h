...
//                cout << "SCREEN OFF 3" <<endl;
        ...
        if(NDer > 0) {
          drhoT[0] += std::real(Tt) * SCRATCH1XDATA[iBf];
          drhoT[1] += std::real(Tt) * SCRATCH1YDATA[iBf];
          drhoT[2] += std::real(Tt) * SCRATCH1ZDATA[iBf];
          if(this->nTCS_ == 1 && !this->isClosedShell){
            drhoS[0] += std::real(Ts) * SCRATCH1XDATA[iBf];
            drhoS[1] += std::real(Ts) * SCRATCH1YDATA[iBf];
            drhoS[2] += std::real(Ts) * SCRATCH1ZDATA[iBf];
          } //UKS
        } //GGA
      } // iBf
    } // iShell
    ...
    for(auto iShell : closeShells) {
      ...
      for(auto iBf = iSt; iBf < (iSt + iSz); iBf++){
        ...
        for(auto jShell : closeShells) {
          int jSz= shSizes[jShell];
          int jSt = this->basisset_->mapSh2Bf(jShell);
          T *VXCDATA = result.VXCScalar.data() + iBf*this->nBasis_;
          for(auto jBf = jSt; jBf < (jSt + jSz); jBf++){
            if(jBf < iBf) continue;
      ...
      for(auto iShell : closeShells) {
        ...
        for(auto iBf = iSt; iBf < (iSt + iSz); iBf++){
          ...
          for(auto jShell : closeShells) {
            int jSz= shSizes[jShell];
            int jSt = this->basisset_->mapSh2Bf(jShell);
            T *VXCDATB = result.VXCMz.data() + iBf*this->nBasis_;
            for(auto jBf = jSt; jBf < (jSt + jSz); jBf++){
              if(jBf < iBf) continue;
...
//NOMAG  KernelIntegrand<double> res(this->vXA_->cols());
  KernelIntegrand<T> res(this->vXCScalar_->cols());
...
//    cout << "Cutoff Rad = " << atomRadCutoff[iAtm] << "for iAt= " << iAtm << endl;
    ...
    std::function<void(IntegrationPoint&,KernelIntegrand<T>&)>
      wrapper = std::bind(valVxc,iAtm,std::placeholders::_1,
                  std::placeholders::_2);

    AGrid.integrate<KernelIntegrand<T>>(wrapper,res);
  };
