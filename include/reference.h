...
namespace ChronusQ {
template<typename T>
class Reference : public Quantum<T> {
...
protected:
  ...
  inline void checkWorkers(){
    if(this->fileio_  == NULL) 
      CErr("Fatal: Must initialize Reference with FileIO Object");
    if(this->basisset_ == NULL) 
      CErr("Fatal: Must initialize Reference with BasisSet Object",
           this->fileio_->out);
    if(this->molecule_ == NULL) 
      CErr("Fatal: Must initialize Reference with Molecule Object",
           this->fileio_->out);
    if(this->aointegrals_== NULL)
      CErr("Fatal: Must initialize Reference with AOIntegrals Object",
           this->fileio_->out);
    if(this->memManager_ == NULL)
      CErr("Fatal: Must initialize Reference with CQMemManager Object",
           this->fileio_->out);
