...
namespace ChronusQ{
void BasisSetNew::basisSetRead(FileIO * fileio, Molecule * mol){
...
void BasisSetNew::findBasisFile(std::string fName){
  std::string tmpStr;
...
void BasisSetNew::constructLocal(Molecule * mol){
...
void BasisSetNew::makeMapSh2Bf(){
  auto n = 0;
  for(auto shell : this->shells_){
     this->mapSh2Bf_.push_back(n);
     n += shell.size();
  }
  this->haveMapSh2Bf_ = true;
}

void BasisSetNew::makeMapSh2Cen(Molecule *mol){
  for(auto shell : this->shells_){
    for(auto iAtom = 0; iAtom < mol->nAtoms(); iAtom++){
  ...
  this->haveMapSh2Cen_ = true;
}

template<>
void BasisSetNew::computeShBlkNorm(bool doBeta, const RealMatrix *DAlpha, 
                                   const RealMatrix *DBeta){
  // If map doesnt exist, make it
  if(!this->haveMapSh2Bf_) this->makeMapSh2Bf();
...
template<>
void BasisSetNew::computeShBlkNorm(bool doBeta, const ComplexMatrix *DAlpha, 
