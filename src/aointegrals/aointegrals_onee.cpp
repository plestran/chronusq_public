...
/*   LA >= LB                       */
/************************************/
double AOIntegrals::oneehRRSab(int LA,int *lA,int LB,int *lB) {
  int i,j;
  double tmpVal = 0.0;
  if(LA+LB==0) {
    for(i=0;i<pairConstants_->nPGTOs[0];i++) for(j=0;j<pairConstants_->nPGTOs[1];j++) tmpVal+= pairConstants_->ssPair[i][j];
    return tmpVal;
  } else if(LB==0) {
    for(i=0;i<pairConstants_->nPGTOs[0];i++) for(j=0;j<pairConstants_->nPGTOs[1];j++) if(pairConstants_->ssNonzero[i][j]) tmpVal+= pairConstants_->ssPair[i][j]*this->oneevRRSa0(LA,lA,&i,&j);
    return tmpVal;
  };
  ...
  if(LB>2) {
	...
	for(i=0;i<pairConstants_->nPGTOs[0];i++) for(j=0;j<pairConstants_->nPGTOs[1];j++)  if(pairConstants_->ssNonzero[i][j]){
	  ...
	  lAp1[iWork]++;
	  primitiveVal += math.two*pairConstants_->deltaAB[iWork]*this->oneevRRSa0(LA+1,lAp1,&i,&j);
	  lAp1[iWork]++;
	  primitiveVal += this->oneevRRSa0(LA+2,lAp1,&i,&j);
	  lAp1[iWork]-=2;
	...
	for(i=0;i<pairConstants_->nPGTOs[0];i++) for(j=0;j<pairConstants_->nPGTOs[1];j++) if(pairConstants_->ssNonzero[i][j]) {
	  primitiveVal = sqrDeltaAB*this->oneevRRSa0(LA,lA,&i,&j);
	  lAp1[iWork]++;
	  primitiveVal += pairConstants_->deltaAB[iWork2]*this->oneevRRSa0(LA+1,lAp1,&i,&j);
	  lAp1[iWork2]++;
	  primitiveVal += this->oneevRRSa0(LA+2,lAp1,&i,&j);
	  lAp1[iWork]--;
	  primitiveVal += pairConstants_->deltaAB[iWork]*this->oneevRRSa0(LA+1,lAp1,&i,&j);
	  lAp1[iWork2]--;
	  tmpVal += primitiveVal*pairConstants_->ssPair[i][j];
	};
      } else {
	...
	tmpVal += pairConstants_->ssPair[i][j]*(this->oneevRRSa0(LA+1,lAp1,&i,&j)+ pairConstants_->deltaAB[iWork]*this->oneevRRSa0(LA,lA,&i,&j));
      ...
      for(i=0;i<pairConstants_->nPGTOs[0];i++) for(j=0;j<pairConstants_->nPGTOs[1];j++)  if(pairConstants_->ssNonzero[i][j]) 
	tmpVal += pairConstants_->ssPair[i][j]*this->oneevRRSa0(LA+1,lAp1,&i,&j);
    };
  };
  return tmpVal;
};
...
/* (a|0) = (P-A)(a-1|0) + 1/(2*(alpha+beta))*N(a-1)*(a-2|0) */
/************************************************************/
double AOIntegrals::oneevRRSa0(int LA,int *lA,int *i,int *j) {
  double tmpVal=0.0;
  int iWork;
  int lAm1[3];
  ...
  if(LA>4) {
    ...
    if(LA==2&&lA[iWork]==2) tmpVal += pairConstants_->Sa0Par[*i][*j];
      ...
      lAm1[iWork] -=1;
      tmpVal += (lAm1[iWork]+1)*pairConstants_->Sa0Par[*i][*j]*this->oneevRRSa0(LA-2,lAm1,i,j);
    };
  } else if (LA==4) {
...
/*   LA >= LB                       */
/************************************/
double AOIntegrals::oneehRRTSab(int LA,int *lA,int LB,int *lB,int *i, int *j) {
  double tmpVal = 0.0;
  if(LA+LB==0) return 1.0;
  else if(LB==0) return this->oneevRRSa0(LA,lA,i,j);
  int iWork,lAp1[3],lBm1[3];
  for(iWork=0;iWork<3;iWork++){
    lAp1[iWork]=lA[iWork];     
    lBm1[iWork]=lB[iWork];     
  ...
  if(LB>2) {
	...
	double sqrDeltaAB = (pairConstants_->deltaAB[iWork])*(pairConstants_->deltaAB[iWork]);
        ...
        lAp1[iWork]++;
        tmpVal += math.two*pairConstants_->deltaAB[iWork]*this->oneevRRSa0(LA+1,lAp1,i,j);
        lAp1[iWork]++;
	...
	tmpVal = sqrDeltaAB*this->oneevRRSa0(LA,lA,i,j);
	lAp1[iWork]++;
	tmpVal += pairConstants_->deltaAB[iWork2]*this->oneevRRSa0(LA+1,lAp1,i,j);
	lAp1[iWork2]++;
	tmpVal += this->oneevRRSa0(LA+2,lAp1,i,j);
	lAp1[iWork]--;
...
/*  from (a|T|b) to (a|T|0)         */
/************************************/
double AOIntegrals::oneevRRTab(int LA,int *lA,int LB,int *lB,int *i,int *j) {
  double tmpVal = 0.0;
  if(LA+LB==0) return pairConstants_->TabPar1[*i][*j];
  else if(LB==0) return this->oneevRRTa0(LA,lA,i,j);
  ...
  if(LB>2) {
    ...
    if(LB==2&&lB[iWork]==2) tmpVal += pairConstants_->Sa0Par[*i][*j]*this->oneevRRTa0(LA,lA,i,j) - pairConstants_->TabPar3[*i][*j]*this->oneevRRSa0(LA,lA,i,j);
    else if (lB[iWork]>=2) {
      lBm1[iWork]--;
      tmpVal += (lBm1[iWork]+1)*pairConstants_->Sa0Par[*i][*j]*this->oneevRRTab(LA,lA,LB-2,lBm1,i,j);
      tmpVal -= (lBm1[iWork]+1)*pairConstants_->TabPar3[*i][*j]*this->oneehRRTSab(LA,lA,LB-2,lBm1,i,j);
    };
  } else if (LB==2) {
    int iWork2;
    if (lBm1[0]>0)      iWork2=0;
	...
	lAm1[iWork2]++;
      ...
      tmpVal += tmpVal2*pairConstants_->deltaPB[iWork][*i][*j];
    };
    if(lA[iWork]>0) {
      lAm1[iWork]--;
...
/*  from (a|T|0) to (0|T|0)         */
/************************************/
double AOIntegrals::oneevRRTa0(int LA,int *lA,int *i,int *j) {
  double tmpVal = 0.0;
  ...
  if(LA>2) {
    ...
    if(LA==2&&lA[iWork]==2) {
      ...
      lAm1[iWork] -=1;
      tmpVal += (lAm1[iWork]+1)*pairConstants_->Sa0Par[*i][*j]*this->oneevRRTa0(LA-2,lAm1,i,j);
      tmpVal -= (lAm1[iWork]+1)*pairConstants_->Ta0Par3[*i][*j]*this->oneevRRSa0(LA-2,lAm1,i,j);
    };
  } else if (LA==2) {
    int iWork2;
    if (lAm1[0]>0)      iWork2=0;
...
/***********************************************/
double AOIntegrals::oneehRRVab(int LA,int *lA,int LB,int *lB){
  int i,j,iAtom;
  ...
  if(LA<=2){
	...
	/**********/
	tmp = 0.0;
	for(i=0;i<pairConstants_->nPGTOs[0];i++) for(j=0;j<pairConstants_->nPGTOs[1];j++)
	...
	for(i=0;i<pairConstants_->nPGTOs[0];i++) for(j=0;j<pairConstants_->nPGTOs[1];j++) if(pairConstants_->ssNonzero[i][j]){
	  B0 = pairConstants_->deltaPA[iWork][i][j];
	...
	if(iWork1!=iWork2) {
	  double B0,B1,B0B1;
	  for(i=0;i<pairConstants_->nPGTOs[0];i++) for(j=0;j<pairConstants_->nPGTOs[1];j++) if(pairConstants_->ssNonzero[i][j]){
	    B0 = pairConstants_->deltaPA[iWork1][i][j];
	    B1 = pairConstants_->deltaPA[iWork2][i][j];
	    B0B1 = B0*B1;
	    for(iAtom=0;iAtom<molecularConstants_->nAtom;iAtom++){
	...
	if(iWork1!=iWork2) {
	  ...
	  for(i=0;i<pairConstants_->nPGTOs[0];i++) for(j=0;j<pairConstants_->nPGTOs[1];j++) if(pairConstants_->ssNonzero[i][j]){
	    A1pB1 = pairConstants_->deltaAB[iWork1]+pairConstants_->deltaPA[iWork1][i][j];
	    B0 = pairConstants_->deltaPA[iWork2][i][j];
	    Coeff0 = A1pB1*B0;
	    for(iAtom=0;iAtom<molecularConstants_->nAtom;iAtom++){
	      tmpVal += molecularConstants_->atomZ[iAtom]*
		(pairConstants_->FmU[0][i][j][iAtom]*Coeff0
	  ...
	  for(i=0;i<pairConstants_->nPGTOs[0];i++) for(j=0;j<pairConstants_->nPGTOs[1];j++) if(pairConstants_->ssNonzero[i][j]){
	    A0pB0 = pairConstants_->deltaAB[iWork1]+pairConstants_->deltaPA[iWork1][i][j];
	    S = pairConstants_->Sa0Par[i][j];
	    Coeff0 = A0pB0*pairConstants_->deltaPA[iWork1][i][j]+S;
	    A0p2B0 = A0pB0+pairConstants_->deltaPA[iWork1][i][j];
	    for(iAtom=0;iAtom<molecularConstants_->nAtom;iAtom++){
	      tmpVal += molecularConstants_->atomZ[iAtom]*
		(pairConstants_->FmU[0][i][j][iAtom]*Coeff0
	...
	if(iWorkA1==iWorkA2&&iWorkB!=iWorkA1) {
	  ...
	  for(i=0;i<pairConstants_->nPGTOs[0];i++) for(j=0;j<pairConstants_->nPGTOs[1];j++) if(pairConstants_->ssNonzero[i][j]) {
	    S = pairConstants_->Sa0Par[i][j];
	    A1pB1  = pairConstants_->deltaAB[iWorkB]+pairConstants_->deltaPA[iWorkB][i][j];
	    B0B0pS = pairConstants_->deltaPA[iWorkA1][i][j]*pairConstants_->deltaPA[iWorkA1][i][j]+S;
	    B02 = math.two*pairConstants_->deltaPA[iWorkA1][i][j];
	    Coeff0 = A1pB1*B0B0pS;
	    for(iAtom=0;iAtom<molecularConstants_->nAtom;iAtom++){
	  ...
	  for(i=0;i<pairConstants_->nPGTOs[0];i++) for(j=0;j<pairConstants_->nPGTOs[1];j++) if(pairConstants_->ssNonzero[i][j]){
	    B0B0    = pairConstants_->deltaPA[iWorkB][i][j]*pairConstants_->deltaPA[iWorkB][i][j];
	    A0p3B0  = pairConstants_->deltaAB[iWorkB]+double(3.0)*pairConstants_->deltaPA[iWorkB][i][j];
	    A02p3B0mB0  = (pairConstants_->deltaAB[iWorkB]+A0p3B0)*pairConstants_->deltaPA[iWorkB][i][j];
	    S = pairConstants_->Sa0Par[i][j];
	    S3 = double(3.0)*S;
	    A0p3B0mS= A0p3B0*S;
	    Coeff0  = (pairConstants_->deltaAB[iWorkB]+pairConstants_->deltaPA[iWorkB][i][j])*B0B0+A0p3B0mS;
	    for(iAtom=0;iAtom<molecularConstants_->nAtom;iAtom++){
	  ...
	  for(i=0;i<pairConstants_->nPGTOs[0];i++) for(j=0;j<pairConstants_->nPGTOs[1];j++) if(pairConstants_->ssNonzero[i][j]){
	    S = pairConstants_->Sa0Par[i][j];
	    B1= pairConstants_->deltaPA[iWorkA1][i][j];
	    A0pB0mB0 = (pairConstants_->deltaAB[iWorkB]+pairConstants_->deltaPA[iWorkB][i][j])*pairConstants_->deltaPA[iWorkB][i][j];
	    A0p2B0   = pairConstants_->deltaAB[iWorkB]+math.two*pairConstants_->deltaPA[iWorkB][i][j];
	    A0p2B0mB1= A0p2B0*B1;
	    B1S      = pairConstants_->deltaPA[iWorkA1][i][j]*S;
	    Coeff0   = A0pB0mB0*pairConstants_->deltaPA[iWorkA1][i][j]+B1S;
	    for(iAtom=0;iAtom<molecularConstants_->nAtom;iAtom++){
	  ...
	  for(i=0;i<pairConstants_->nPGTOs[0];i++) for(j=0;j<pairConstants_->nPGTOs[1];j++) if(pairConstants_->ssNonzero[i][j]){
	    B0 = pairConstants_->deltaPA[iWorkA1][i][j];
	    B1 = pairConstants_->deltaPA[iWorkA2][i][j];
	    B0B1      = B0*B1;
	    A2pB2     = pairConstants_->deltaAB[iWorkB]+pairConstants_->deltaPA[iWorkB][i][j];
	    Coeff0    = A2pB2*B0B1;
	    for(iAtom=0;iAtom<molecularConstants_->nAtom;iAtom++){
	...
	for(i=0;i<pairConstants_->nPGTOs[0];i++) for(j=0;j<pairConstants_->nPGTOs[1];j++) if(pairConstants_->ssNonzero[i][j]) {
	  ...
	  B0 = pairConstants_->deltaPA[iWorkA1][i][j];
	  B02 = math.two*B0;
	  A1pB1      = pairConstants_->deltaAB[iWorkB1]+pairConstants_->deltaPA[iWorkB1][i][j];
	...
	for(i=0;i<pairConstants_->nPGTOs[0];i++) for(j=0;j<pairConstants_->nPGTOs[1];j++) if(pairConstants_->ssNonzero[i][j]) {
	  S = pairConstants_->Sa0Par[i][j];
	  A0A0      = pairConstants_->deltaAB[iWorkA1]*pairConstants_->deltaAB[iWorkA1];
	  B0B0      = pairConstants_->deltaPA[iWorkA1][i][j]*pairConstants_->deltaPA[iWorkA1][i][j];
	  A0pB0mB0  = (pairConstants_->deltaAB[iWorkA1]+pairConstants_->deltaPA[iWorkA1][i][j])*pairConstants_->deltaPA[iWorkA1][i][j];
	  A0p2B0    = pairConstants_->deltaAB[iWorkA1]+math.two*pairConstants_->deltaPA[iWorkA1][i][j];
	  sum1      = A0p2B0*A0p2B0+math.two*A0pB0mB0+double(3.0)*pairConstants_->Sa0Par[i][j];
	  Coeff0    = A0pB0mB0*A0pB0mB0+sum1*S;
	  for(iAtom=0;iAtom<molecularConstants_->nAtom;iAtom++){
	    A0p2B0m2C0= math.two*(pairConstants_->deltaAB[iWorkA1]+math.two*pairConstants_->deltaPA[iWorkA1][i][j])*pairConstants_->deltaPZ[iWorkA1][i][j][iAtom];
	    C0C0      = pairConstants_->deltaPZ[iWorkA1][i][j][iAtom]*pairConstants_->deltaPZ[iWorkA1][i][j][iAtom];
	...
	for(i=0;i<pairConstants_->nPGTOs[0];i++) for(j=0;j<pairConstants_->nPGTOs[1];j++) if(pairConstants_->ssNonzero[i][j]) {
	  S = pairConstants_->Sa0Par[i][j];
	  A0pB0  = pairConstants_->deltaAB[iWorkA1]+pairConstants_->deltaPA[iWorkA1][i][j];
	  A0p3B0 = pairConstants_->deltaAB[iWorkA1]+double(3.0)*pairConstants_->deltaPA[iWorkA1][i][j];
	  A02p3B0= math.two*pairConstants_->deltaAB[iWorkA1]+double(3.0)*pairConstants_->deltaPA[iWorkA1][i][j];
	  A1pB1  = pairConstants_->deltaAB[iWorkB2]+pairConstants_->deltaPA[iWorkB2][i][j];
	  B0B0   = pairConstants_->deltaPA[iWorkA1][i][j]*pairConstants_->deltaPA[iWorkA1][i][j];
	  sum1   = A0pB0*B0B0+A0p3B0*S;
	  Coeff0 = A1pB1*sum1;
	  for(iAtom=0;iAtom<molecularConstants_->nAtom;iAtom++){
	    sum2   = (A0p3B0+double(3.0)*pairConstants_->deltaPZ[iWorkA1][i][j][iAtom])*S;
	    B0C0   = pairConstants_->deltaPA[iWorkA1][i][j]*pairConstants_->deltaPZ[iWorkA1][i][j][iAtom];
	    C0C0   = pairConstants_->deltaPZ[iWorkA1][i][j][iAtom]*pairConstants_->deltaPZ[iWorkA1][i][j][iAtom];
	...
	for(i=0;i<pairConstants_->nPGTOs[0];i++) for(j=0;j<pairConstants_->nPGTOs[1];j++) if(pairConstants_->ssNonzero[i][j]) {
	  S = pairConstants_->Sa0Par[i][j];
	  A0pB0  = -pairConstants_->deltaAB[iWorkB1]+pairConstants_->deltaPB[iWorkB1][i][j];
	  A0p3B0 = -pairConstants_->deltaAB[iWorkB1]+double(3.0)*pairConstants_->deltaPB[iWorkB1][i][j];
	  A02p3B0= -math.two*pairConstants_->deltaAB[iWorkB1]+double(3.0)*pairConstants_->deltaPB[iWorkB1][i][j];
	  A1pB1  = -pairConstants_->deltaAB[iWorkA2]+pairConstants_->deltaPB[iWorkA2][i][j];
	  B0B0   = pairConstants_->deltaPB[iWorkB1][i][j]*pairConstants_->deltaPB[iWorkB1][i][j];
	  sum1   = A0pB0*B0B0+A0p3B0*pairConstants_->Sa0Par[i][j];
	  Coeff0 = A1pB1*sum1;
	  for(iAtom=0;iAtom<molecularConstants_->nAtom;iAtom++){
	    sum2   = (A0p3B0+double(3.0)*pairConstants_->deltaPZ[iWorkB1][i][j][iAtom])*S;
	    B0C0   = pairConstants_->deltaPB[iWorkB1][i][j]*pairConstants_->deltaPZ[iWorkB1][i][j][iAtom];
	...
	for(i=0;i<pairConstants_->nPGTOs[0];i++) for(j=0;j<pairConstants_->nPGTOs[1];j++) if(pairConstants_->ssNonzero[i][j]) {
	  S = pairConstants_->Sa0Par[i][j];
	  A1pB1 = pairConstants_->deltaAB[iWorkB1]+pairConstants_->deltaPA[iWorkB1][i][j];
	  A2pB2 = pairConstants_->deltaAB[iWorkB2]+pairConstants_->deltaPA[iWorkB2][i][j];
	  prod1 = A1pB1*A2pB2;
	  B02pS = pairConstants_->deltaPA[iWorkA1][i][j]*pairConstants_->deltaPA[iWorkA1][i][j]+S;
	  Coeff0= prod1*B02pS;
	  for(iAtom=0;iAtom<molecularConstants_->nAtom;iAtom++){
	    B0C0pS= math.two*pairConstants_->deltaPA[iWorkA1][i][j]*pairConstants_->deltaPZ[iWorkA1][i][j][iAtom]+S;
	...
	for(i=0;i<pairConstants_->nPGTOs[0];i++) for(j=0;j<pairConstants_->nPGTOs[1];j++) if(pairConstants_->ssNonzero[i][j]){
	  S = pairConstants_->Sa0Par[i][j];
	  A1pB1 = -pairConstants_->deltaAB[iWorkA1]+pairConstants_->deltaPB[iWorkA1][i][j];
	  A2pB2 = -pairConstants_->deltaAB[iWorkA2]+pairConstants_->deltaPB[iWorkA2][i][j];
	  prod1 = A1pB1*A2pB2;
	  B02pS = pairConstants_->deltaPB[iWorkB1][i][j]*pairConstants_->deltaPB[iWorkB1][i][j]+S;
	  Coeff0= prod1*B02pS;
	  for(iAtom=0;iAtom<molecularConstants_->nAtom;iAtom++){
	    B0C0pS= math.two*pairConstants_->deltaPB[iWorkB1][i][j]*pairConstants_->deltaPZ[iWorkB1][i][j][iAtom]+S;
	...
	for(i=0;i<pairConstants_->nPGTOs[0];i++) for(j=0;j<pairConstants_->nPGTOs[1];j++) if(pairConstants_->ssNonzero[i][j]) {
	  S = pairConstants_->Sa0Par[i][j];
	  A0pB0mB0 = (pairConstants_->deltaAB[iWorkA1]+pairConstants_->deltaPA[iWorkA1][i][j])*pairConstants_->deltaPA[iWorkA1][i][j];
	  A1pB1mB1 = (pairConstants_->deltaAB[iWorkA2]+pairConstants_->deltaPA[iWorkA2][i][j])*pairConstants_->deltaPA[iWorkA2][i][j];
	  sum1     = (A0pB0mB0+A1pB1mB1)*S;
	  SS       = S*S;
	  Coeff0   = A0pB0mB0*A1pB1mB1+sum1+SS;
	  for(iAtom=0;iAtom<molecularConstants_->nAtom;iAtom++){
	    C0mA02B0 = pairConstants_->deltaPZ[iWorkA1][i][j][iAtom]*(pairConstants_->deltaAB[iWorkA1]+math.two*pairConstants_->deltaPA[iWorkA1][i][j]);
	    C1mA12B1 = pairConstants_->deltaPZ[iWorkA2][i][j][iAtom]*(pairConstants_->deltaAB[iWorkA2]+math.two*pairConstants_->deltaPA[iWorkA2][i][j]);
	...
	for(i=0;i<pairConstants_->nPGTOs[0];i++) for(j=0;j<pairConstants_->nPGTOs[1];j++) if(pairConstants_->ssNonzero[i][j]) {
	  S = pairConstants_->Sa0Par[i][j];
	  A1pB1mB1 = (pairConstants_->deltaAB[iWorkB1]+pairConstants_->deltaPA[iWorkB1][i][j])*pairConstants_->deltaPA[iWorkB1][i][j];
	  A2pB2    = pairConstants_->deltaAB[iWorkB2]+pairConstants_->deltaPA[iWorkB2][i][j];
	  A2pB2mB0 = A2pB2*pairConstants_->deltaPA[iWorkA1][i][j];
	  Coeff0   = (A1pB1mB1+pairConstants_->Sa0Par[i][j])*A2pB2mB0;
	  for(iAtom=0;iAtom<molecularConstants_->nAtom;iAtom++){
	    A2pB2mC0 = A2pB2*pairConstants_->deltaPZ[iWorkA1][i][j][iAtom];
	    B0C2     = pairConstants_->deltaPA[iWorkA1][i][j]*pairConstants_->deltaPZ[iWorkB2][i][j][iAtom];
	    A1p2B1mC1= (pairConstants_->deltaAB[iWorkB1]+math.two*pairConstants_->deltaPA[iWorkB1][i][j])*pairConstants_->deltaPZ[iWorkB1][i][j][iAtom];
	    C1C1     = pairConstants_->deltaPZ[iWorkB1][i][j][iAtom]*pairConstants_->deltaPZ[iWorkB1][i][j][iAtom];
...
/***********************************************/
double AOIntegrals::oneevRRVa0(int *iAtom,int m,int LA,int *lA, int *i, int *j){
  ...
  if(LA>3){
    ...
    if(LA==2&&lA[iWork]==2)
      ...
      lAm1[iWork] -=1;
      tmpVal += (lAm1[iWork]+1)*pairConstants_->Sa0Par[*i][*j]*(this->oneevRRVa0(iAtom,m,LA-2,lAm1,i,j)-this->oneevRRVa0(iAtom,m+1,LA-2,lAm1,i,j));
    };
  } else if (LA==3) {
    if(lA[0]==lA[1]&&lA[1]==lA[2]) {
      ...
      tmpVal += pairConstants_->FmU[m][*i][*j][*iAtom]*pairConstants_->deltaPA[iWork][*i][*j]*pairConstants_->deltaPA[iWork2][*i][*j]*pairConstants_->deltaPA[iWork3][*i][*j];
      tmpVal -= pairConstants_->FmU[m+1][*i][*j][*iAtom]*(pairConstants_->deltaPA[iWork][*i][*j]*pairConstants_->deltaPA[iWork2][*i][*j]*pairConstants_->deltaPZ[iWork3][*i][*j][*iAtom]+pairConstants_->deltaPA[iWork][*i][*j]*pairConstants_->deltaPA[iWork3][*i][*j]*pairConstants_->deltaPZ[iWork2][*i][*j][*iAtom]+pairConstants_->deltaPA[iWork2][*i][*j]*pairConstants_->deltaPA[iWork3][*i][*j]*pairConstants_->deltaPZ[iWork][*i][*j][*iAtom]);
      tmpVal += pairConstants_->FmU[m+2][*i][*j][*iAtom]*(pairConstants_->deltaPA[iWork][*i][*j]*pairConstants_->deltaPZ[iWork2][*i][*j][*iAtom]*pairConstants_->deltaPZ[iWork3][*i][*j][*iAtom]+pairConstants_->deltaPA[iWork2][*i][*j]*pairConstants_->deltaPZ[iWork][*i][*j][*iAtom]*pairConstants_->deltaPZ[iWork3][*i][*j][*iAtom]+pairConstants_->deltaPA[iWork3][*i][*j]*pairConstants_->deltaPZ[iWork][*i][*j][*iAtom]*pairConstants_->deltaPZ[iWork2][*i][*j][*iAtom]);
      ...
      double sqrPA = pairConstants_->deltaPA[iWork][*i][*j]*pairConstants_->deltaPA[iWork][*i][*j];
      double sqrPZ = pairConstants_->deltaPZ[iWork][*i][*j][*iAtom]*pairConstants_->deltaPZ[iWork][*i][*j][*iAtom];
      tmpVal += pairConstants_->FmU[m][*i][*j][*iAtom]*pairConstants_->deltaPA[iWork][*i][*j]*(sqrPA+math.three*pairConstants_->Sa0Par[*i][*j]);
      tmpVal -= pairConstants_->FmU[m+1][*i][*j][*iAtom]*math.three*(pairConstants_->deltaPZ[iWork][*i][*j][*iAtom]*sqrPA+pairConstants_->Sa0Par[*i][*j]*(pairConstants_->deltaPA[iWork][*i][*j]+pairConstants_->deltaPZ[iWork][*i][*j][*iAtom]));
      tmpVal += pairConstants_->FmU[m+2][*i][*j][*iAtom]*math.three*(pairConstants_->deltaPA[iWork][*i][*j]*sqrPZ+pairConstants_->Sa0Par[*i][*j]*pairConstants_->deltaPZ[iWork][*i][*j][*iAtom]);
	...
	iWork=iWork3;
      ...
      double sqrPA = pairConstants_->deltaPA[iWork][*i][*j]*pairConstants_->deltaPA[iWork][*i][*j];
      double sqrPZ = pairConstants_->deltaPZ[iWork][*i][*j][*iAtom]*pairConstants_->deltaPZ[iWork][*i][*j][*iAtom];
      double PAPZ  = pairConstants_->deltaPA[iWork][*i][*j]*pairConstants_->deltaPZ[iWork][*i][*j][*iAtom];
      tmpVal += pairConstants_->FmU[m][*i][*j][*iAtom]*pairConstants_->deltaPA[iWork2][*i][*j]*(sqrPA+pairConstants_->Sa0Par[*i][*j]);
      tmpVal -= pairConstants_->FmU[m+1][*i][*j][*iAtom]*(pairConstants_->deltaPZ[iWork2][*i][*j][*iAtom]*(sqrPA+pairConstants_->Sa0Par[*i][*j])+pairConstants_->deltaPA[iWork2][*i][*j]*(pairConstants_->Sa0Par[*i][*j]+math.two*PAPZ));
      tmpVal += pairConstants_->FmU[m+2][*i][*j][*iAtom]*(pairConstants_->deltaPA[iWork2][*i][*j]*sqrPZ+pairConstants_->deltaPZ[iWork2][*i][*j][*iAtom]*(pairConstants_->Sa0Par[*i][*j]+math.two*PAPZ));
    ...
    if(iWork!=iWork2) {
      tmpVal += pairConstants_->FmU[m][*i][*j][*iAtom]*pairConstants_->deltaPA[iWork][*i][*j]*pairConstants_->deltaPA[iWork2][*i][*j];
      tmpVal -= pairConstants_->FmU[m+1][*i][*j][*iAtom]*(pairConstants_->deltaPZ[iWork2][*i][*j][*iAtom]*pairConstants_->deltaPA[iWork][*i][*j]+pairConstants_->deltaPA[iWork2][*i][*j]*pairConstants_->deltaPZ[iWork][*i][*j][*iAtom]);
      tmpVal += pairConstants_->FmU[m+2][*i][*j][*iAtom]*pairConstants_->deltaPZ[iWork][*i][*j][*iAtom]*pairConstants_->deltaPZ[iWork2][*i][*j][*iAtom];
    } else {
      tmpVal += pairConstants_->FmU[m][*i][*j][*iAtom]*(pairConstants_->deltaPA[iWork][*i][*j]*pairConstants_->deltaPA[iWork][*i][*j]+pairConstants_->Sa0Par[*i][*j]);
      tmpVal -= pairConstants_->FmU[m+1][*i][*j][*iAtom]*(math.two*pairConstants_->deltaPA[iWork][*i][*j]*pairConstants_->deltaPZ[iWork][*i][*j][*iAtom]+pairConstants_->Sa0Par[*i][*j]);
      tmpVal += pairConstants_->FmU[m+2][*i][*j][*iAtom]*pairConstants_->deltaPZ[iWork][*i][*j][*iAtom]*pairConstants_->deltaPZ[iWork2][*i][*j][*iAtom];
    };
   } else{
    tmpVal  = pairConstants_->FmU[m][*i][*j][*iAtom]*pairConstants_->deltaPA[iWork][*i][*j];
