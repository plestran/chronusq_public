...
*/
  ...
  if (this->isDFT) energyXC = (3.0/4.0)*((*this->vXCA_).frobInner(this->densityA_->conjugate()));
  if(!this->isClosedShell && this->Ref_ != TCS){
    ...
    if(this->isDFT){
      energyXC  = (3.0/4.0)*((*this->vXCA_).frobInner((this->densityA_->conjugate())));
      energyXC += (3.0/4.0)*((*this->vXCB_).frobInner((this->densityB_->conjugate())));
      }
     }
  if(this->isDFT) this->energyTwoE += energyXC;
