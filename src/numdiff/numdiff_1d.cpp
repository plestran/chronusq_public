...
*/
    ...
    // SCF energy gradient
    double gsdx = (scf_p - scf_m) / (2*(XP-X));
    ...
    Eigen::VectorXd freqDX = (freq_p - freq_m)/(2*(XP-X));
   ...
   if(respType == RESPONSE_TYPE::CIS){
     NAC_ES_GS = ES_GS_NACME_CIS(nFreq,true,NOCC,NVIR,XP-X,
       T_0,(*ss_0.moA()),(*ss_p.moA()),(*ss_m.moA()),
       S_0_p,S_0_m);
     NAC_ES_ES = ES_ES_NACME_CIS(nFreq,true,NOCC,NVIR,XP-X,
     ...
     NAC_ES_ES = ES_ES_NACME_PPTDA(nFreq,true,NOCC,NVIR,XP-X,
       T_0,T_p,T_m,(*ss_0.moA()),(*ss_p.moA()),
       (*ss_m.moA()),S_0_p,S_0_m);
   }
   ...
   if(debug){
     ...
     if(respType == RESPONSE_TYPE::CIS) {
       NAC_GS_ES = GS_ES_NACME_CIS(nFreq,false,NOCC,NVIR,XP-X,
         T_0,T_p,T_m,(*ss_0.moA()),(*ss_p.moA()),
         (*ss_m.moA()),S_0_p,S_0_m);
