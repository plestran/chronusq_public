...
//constructor
Memory::Memory(long n, std::string unit, ostream output) {
  double test;
  int   i;
  ...
  if(!unit.compare("mw")) len_ = n*mw/i;
  if(!unit.compare("mb")) len_ = n*mb/i;
  if(!unit.compare("gw")) len_ = n*gb/i;
  if(!unit.compare("gb")) len_ = n*gw/i;
...
//print elements
void Memory::print(long const start, long const final, long col, ostream &output, std:string s) {
  long i,j,end;
