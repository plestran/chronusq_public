...
// Form the initial density by swaping MOs
    ...
    if (this->swapMOB_ != 0 && !this->isClosedShell_ && this->Ref_ != SingleSlater<dcomplex>::TCS) {
...
// Transform density from orthonormal to AO basis
    *this->ssPropagator_->densityA() = (*this->oTrans1_)*(*this->POAsav_)*(*this->oTrans1_);
    if(!this->isClosedShell_ && this->Ref_ != SingleSlater<dcomplex>::TCS) *this->ssPropagator_->densityB() = (*this->oTrans1_)*(*this->POB_)*(*this->oTrans1_);
  }
};
